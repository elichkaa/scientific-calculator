...
{
  String result;
  while(checkIfSignIsContained('/', expr) == 1)
  {
  ...
  while(checkIfSignIsContained('*', expr) == 1)
  {
    result = doBodmasFromString(expr, '*', &multiplication);
    expr = result;
  }
  while(checkIfSignIsContained('+', expr) == 1)
  {
    result = doBodmasFromString(expr, '+', &addition);
    expr = result;
  }
  while(checkIfSignIsContained('-',expr) == 1)
  {
    result = doBodmasFromString(expr, '-', &subtraction);
    expr = result;
  }
  return result;
}
